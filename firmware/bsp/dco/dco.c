...
static inline bool setup_pll(uint8_t a, uint32_t b, uint32_t c) __attribute__((always_inline));
static inline bool setup_ms(uint8_t i_div, uint8_t r_div)       __attribute__((always_inline));
...
bool dco_start(uint32_t frequency) {
    ...
    return (i2c_master_write(SI5351_I2C_ADDR, xtal_load, sizeof(xtal_load)) == sizeof(xtal_load)) &&
...
bool dco_set_frequency(uint32_t frequency) {
    ...
    if(frequency >= DCO_MIN_FREQUENCY && frequency <= DCO_MAX_FREQUENCY) {
        ...
        result =  setup_pll(pll_f / DCO_REF_FREQUENCY, ((uint64_t)(pll_f % DCO_REF_FREQUENCY) * SI5351_FLOAT_MAX) / DCO_REF_FREQUENCY, SI5351_FLOAT_MAX) && setup_ms(div, 0);
    }
    return result;
}
...
static inline bool setup_pll(uint8_t a, uint32_t b, uint32_t c) {
...
static inline bool setup_ms(uint8_t i_div, uint8_t r_div) {
